...
1.  What type of study do you think these data come from: observational or experiment?
    Why?
`I think it is observational. I don't think that is it practical for the study to have people smoke for years to see if they live. It was more likely a survey of the population.` 

2.  How many observations are in this dataset?
    What does each observation represent?
`There are 1314 observations. Each observation is a person.`
...
3.  How many variables are in this dataset?
    ...
    `There are 3 variables. Outcome and Smoker are logical variables. Age is an integer.`

```{r table, eval = TRUE}
Whickham %>%
  ggplot(aes(x = outcome, fill = smoker, color = smoker)) +
  geom_bar(position = "fill") +
  labs(
    title = "Smoker vs Morality",
    x = "Outcome",
    y = "Count",
    fill = "Smoker",
    color = "Smoker"
...
```
```{r age-graph, eval = TRUE}
Whickham %>%
  ggplot(aes(x = age, color = smoker, fill = smoker)) +
  geom_histogram()+
  labs(
    title = "Age of Smokers",
    x = "Age",
    y = "Count",
    fill = "Smoker",
    color = "Smoker"
...
`I would expect that smoking would negatively affect health.`
